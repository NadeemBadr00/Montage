...
import { sliceAudioBuffer, bufferToWave, arrayBufferToBase64, timeStringToSeconds, secondsToSRTTime, applySubtitlesToTimeline } from './ai-utils';
...
class AIManager {
    ...
    // --- دوال مساعدة للصوت (تفويض للدوال المستوردة) ---

    sliceAudioBuffer(fullBuffer, startSec, endSec, ctx) { return sliceAudioBuffer(fullBuffer, startSec, endSec, ctx); }
    bufferToWave(abuffer, len) { return bufferToWave(abuffer, len); }
    arrayBufferToBase64(buffer) { return arrayBufferToBase64(buffer); }

    // --- دوال مساعدة للوقت والتنسيق (تفويض للدوال المستوردة) ---

    timeStringToSeconds(timeStr) { return timeStringToSeconds(timeStr); }
    secondsToSRTTime(seconds) { return secondsToSRTTime(seconds); }

    // 🔥 FIX: الإصلاح الجوهري للترجمة الآلية (SRT) مع React / Zustand
    applySubtitlesToTimeline(srtContent) {
        this.lastGeneratedSRT = srtContent;
        return applySubtitlesToTimeline(srtContent);
    }
