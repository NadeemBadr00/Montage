...
export function drawAdvancedText(ctx: CanvasRenderingContext2D, clip: any, w: number, h: number) {
    ...
    const hasBg = bgColor && bgColor !== 'transparent' && bgColor !== '#00000000';
    if (hasBg) {
        ctx.save();
        const bgOpacity = style.backgroundOpacity !== undefined ? style.backgroundOpacity / 100 : 1;
        ctx.globalAlpha *= bgOpacity;
        ctx.fillStyle = forceHex(bgColor);
